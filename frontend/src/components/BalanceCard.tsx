...
export default function BalanceCard({
  ...
  // Get total collateral value (only on Arbitrum)
  ...
  const formattedCollateral = collateralValueUSD ? formatUnits(collateralValueUSD, 6) : '0';
      ...
      <div className="space-y-3">
        ...
        {chainKey === 'arbitrumSepolia' && (
          <div className="bg-white/10 rounded-lg p-3 border border-white/20">
            <p className="text-white/60 text-xs mb-2 font-semibold">🏦 Borrowing Position</p>
            <div className="space-y-1">
              <div className="flex justify-between">
                <span className="text-white/80 text-sm">Collateral</span>
                <span className="text-white font-mono text-sm font-bold">
                  ${parseFloat(formattedCollateral).toFixed(2)}
                </span>
              </div>
              ...
              {hasActiveLoan ? (
                <>
                  <div className="flex justify-between">
                    <span className="text-red-300 text-sm">Debt</span>
                ...
                <div className="text-white/40 text-xs text-center mt-1">No active loan</div>
              )}
            </div>
          </div>
        )}
